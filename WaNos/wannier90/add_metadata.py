...
class UtilTricks:
    ...
    def get_loaded_modules():
        ...
        """
        # First try to get LOADEDMODULES environment variable
        loaded_modules_env = os.environ.get('LOADEDMODULES')
        if loaded_modules_env:
            # LOADEDMODULES is a colon-separated list
            modules_list = loaded_modules_env.split(':')
            return ', '.join(modules_list)
        else:
            # Try to use the 'module' command
            try:
                import subprocess
                # Using 'module' command and capturing stderr because module outputs to stderr
                loaded_modules = subprocess.check_output('module list', stderr=subprocess.STDOUT, universal_newlines=True, shell=True)
                return loaded_modules.strip()
            except FileNotFoundError:
                # module command not found, silently return
                return 'Module command not found'
            except Exception as e:
